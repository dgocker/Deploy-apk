...
6.  **Настройте адрес сервера**:
    *   Создайте еще один секрет с именем `VITE_API_URL`.
    *   Вставьте туда URL вашего развернутого сервера (например, `https://ваш-проект.run.app`). Это необходимо, чтобы мобильное приложение знало, куда отправлять запросы.
